...
class MysqliDb
{
     ...
     * @var string
     */
    protected $tableLockMethod = "READ";
     ...
     * @var bool
     */
    protected $lockInShareMode = false;
     ...
     * @param string $socket
     */
    public function __construct($host = null, $username = null, $password = null, $database = null, $port = null, $charset = 'utf8', $socket = null)
    {
        $isSubQuery = false;
    ...
    public function connect($connectionName = 'default')
    {
        ...
        $this->mysqli[$connectionName] = $mysqli;
    }
    ...
    {
        foreach (array_keys($this->mysqli) as $k) {
            $this->disconnect($k);
        }
    }
    ...
    {
        if (!isset($this->mysqli[$connection]))
            return;

        $this->mysqli[$connection]->close();
        unset($this->mysqli[$connection]);
    }
    ...
    {
        if (!isset($this->mysqli[$this->defConnectionName])) {
            $this->connect($this->defConnectionName);
        }
        return $this->mysqli[$this->defConnectionName];
    }
     ...
     *
     * @uses $database = MySqliDb::getInstance();
     *
    ...
    protected function reset()
    {
        if ($this->traceEnabled) {
            $this->trace[] = array($this->lastQuery, (microtime(true) - $this->traceStartQ), $this->_traceGetCaller());
        }
    ...
    public function rawQuery($query, $bindParams = null)
    {
        ...
        $this->lastQuery = $this->replacePlaceHolders($this->query, $params);
        ...
        if(gettype($table) == "array") {
            ...
            foreach($table as $key => $value) {
                if(gettype($value) == "string") {
                    ...
                    $this->query .= " ".self::$prefix.$value." ".$this->tableLockMethod;
                }
            }
        }
        else{
            ...
            // Build the query
            $this->query = "LOCK TABLES ".$table." ".$this->tableLockMethod;
        }

        // Execute the query unprepared because LOCK only works with unprepared statements.
        $result = $this->queryUnprepared($this->query);
    ...
    {
        $this->bindParams[0] .= $this->_determineType($value);
    ...
    private function _buildInsert($tableName, $insertData, $operation)
    {
        ...
        $this->query = $operation . " " . implode(' ', $this->queryOptions) . " INTO " . self::$prefix . $tableName;
    ...
    protected function _buildQuery($numRows = null, $tableData = null)
    {
        ...
        $this->_buildCondition('WHERE', $this->where);
        $this->_buildGroupBy();
        $this->_buildCondition('HAVING', $this->having);
        $this->_buildOrderBy();
        ...
        if (count($this->bindParams) > 1) {
            call_user_func_array(array($stmt, 'bind_param'), $this->refValues($this->bindParams));
        }
    ...
    protected function _dynamicBindResults(\mysqli_stmt $stmt)
    {
        ...
        while ($field = $meta->fetch_field()) {
            ...
            if ($this->nestJoin && $field->table != $this->tableName) {
    ...
    public function _buildDataPairs($tableData, $tableColumns, $isInsert)
    {
        foreach ($tableColumns as $column) {
            ...
            if ($value instanceof MysqliDb) {
                $this->query .= $this->_buildPair("", $value) . ", ";
                continue;
            }
    ...
    protected function _buildCondition($operator, &$conditions)
    {
        ...
        foreach ($conditions as $cond) {
            ...
            switch (strtolower($operator)) {
                ...
                case 'not exists':
                case 'exists':
                    $this->query.= $operator . $this->_buildPair("", $val);
                    break;
                default:
                    if (is_array($val)) {
                        $this->_bindParams($val);
                        ...
                        $this->query .= $this->_buildPair($operator, $val);
                    }
            }
        }
    }
    ...
    public function getLastError()
    {
        ...
        return trim($this->stmtError . " " . $this->mysqli()->error);
    }
    ...
    private function _traceGetCaller()
    {
        ...
        return "MysqliDb ->" . $caller["function"] . "() >>  file \"" .
    ...
    public function tableExists($tables)
    {
        ...
        $database = isset($this->connectionsSettings[$this->defConnectionName]) ? $this->connectionsSettings[$this->defConnectionName]['db'] : null;
     ...
     *
     * @uses $databaseWrapper->joinWhere('user u', 'u.id', 7)->where('user u', 'u.title', 'MyTitle');
     *
    ...
    {
        $this->joinAnd[self::$prefix . $whereJoin][] = Array ($cond, $whereProp, $operator, $whereValue);
        return $this;
    }
